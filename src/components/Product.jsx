...
function Product({ product }) {
    ...
    return (
        <div>
            <div onClick={() => navigate("/product-details/" + id)} className='w-[220px] h-[460px] shadow-lg shadow-gray-400 rounded-lg m-3 hover:shadow-2xl transition duration-300 cursor-pointer'>
                <img className='w-48 h-64' src={image} alt="product_image" />
                <div>
                    <p className='text-center h-12'>{title}</p>
                    <Rating className='mt-14' value={rating.rate} onChange={(e) => setValue(e.rating.rate)} readOnly cancel={false} />
